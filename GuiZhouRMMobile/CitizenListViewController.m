...
{
    if ([self.citizenListPopover isPopoverVisible]) {
        [self.citizenListPopover dismissPopoverAnimated:animated];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        if ([tableView isEditing]) {
            ...
            if (textField == nil) {
                textField=[[UITextField alloc] initWithFrame:CGRectMake(cell.frame.origin.x+6, (cell.frame.size.height-31)/2, cell.frame.size.width-100, 31)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self.delegate  setAutoNumber:cell.textLabel.text];
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        NSString *selectedAutoNumber=[_delegate getCitizenNameDelegate];
        if ([selectedAutoNumber isEqualToString:temp.automobile_number]) {
            [self.delegate clearCitizenInfo];
        }
    ...
    if (editingStyle == UITableViewCellEditingStyleInsert) {
        ...
        Citizen *newCitizen=[[Citizen alloc] initWithEntity:entity insertIntoManagedObjectContext:context];
...
- (IBAction)btnEdit:(id)sender {
    if ([self.citizenListView isEditing]) {
        ...
        for (int i=0; i<self.citizenList.count; i++) {
            ...
            [[myCell.contentView viewWithTag:1007] removeFromSuperview];
        }
        ...
        for (int i=0; i<self.citizenList.count; i++) {
            ...
            for (int j=0; j<self.citizenList.count; j++) {
                ...
                if (([citizeni.automobile_number isEqualToString:citizenj.automobile_number]) && ([citizeni.caseinfo_id isEqualToString:citizenj.caseinfo_id]) && (i!=j)) {
                    NSManagedObjectContext *context=[[AppDelegate App] managedObjectContext];
...
-(void)deleteCitizenInfo:(Citizen *)citizen{
    ...
    if (numberOfCitizen-1>1) {
        ...
        entity=[NSEntityDescription entityForName:@"CaseDeformation" inManagedObjectContext:context];
        predicate=[NSPredicate predicateWithFormat:@"caseinfo_id==%@ && (citizen_name==%@ || citizen_name==%@)",caseID,citizenName,@"共同"];    
        [fetchRequest setEntity:entity];
        [fetchRequest setPredicate:predicate];
        tempArray=[context executeFetchRequest:fetchRequest error:nil];
        for (NSManagedObject *obj in tempArray) {
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    NSIndexPath *indexPath = [self.citizenListView indexPathForCell:cell];
    ...
    Citizen *citizen=[self.citizenList objectAtIndex:indexPath.row];
    if (citizen) {
        NSString *autoNumberNew=textField.text;
        citizen.automobile_number=autoNumberNew;
        citizen.caseinfo_id=self.caseID;
        [[AppDelegate App] saveContext];
    }
