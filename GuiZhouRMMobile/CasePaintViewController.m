...
@synthesize moveableImageArray=_moveableImageArray;
...
- (void)viewDidLoad{
    ...
    self.iconTypeArray=@[@"模版",@"车辆",@"路产",@"绘图"];
    ...
    self.roadModelBoard.iconModelID = @"";
    self.roadModelBoard.contentMode = UIViewContentModeCenter;
...
{
    if ([self.textInputPopover isPopoverVisible]) {
        [self.textInputPopover dismissPopoverAnimated:animated];
    }
...
-(void)deleteMoveableImage:(MoveableImage *)moveableImage{
    [self.moveableImageArray removeObject:moveableImage];
}
...
-(void)inputString:(NSString *)text fontSize:(CGFloat)fontSize inRect:(CGRect)rect{
    ...
    moveableImage.contentMode = UIViewContentModeCenter;
    ...
    moveableImage.fontSize = drawFontSize;
    UIGraphicsEndImageContext();
            ...
            @autoreleasepool {
                ...
                if (model.items != nil) {
                    ...
                    for (IconItems *item in iconItems) {
                        ...
                        [stroke addPathWithType:item.itemtype.integerValue P1:point1 P2:point2];
    ...
    if (![self.caseID isEmpty]) {
        ...
        if (caseMap.map_item && ![caseMap.map_item isEmpty]) {
            ...
            for (DDXMLElement *item in foreGroundItemsArray) {
                CGPoint point1,point2;
                point1.x = [item attributeForName:@"X1"].stringValue.floatValue;
                point1.y = [item attributeForName:@"Y1"].stringValue.floatValue;
                point2.x = [item attributeForName:@"X2"].stringValue.floatValue;
                point2.y = [item attributeForName:@"Y2"].stringValue.floatValue;
                NSInteger itemType = [item attributeForName:@"Type"].stringValue.integerValue;
                DWStroke *stroke=[[DWStroke alloc] init];
                [stroke addPathWithType:itemType P1:point1 P2:point2];                
            ...
            for (DDXMLElement *text in foreGroundTextsArray) {
                CGFloat textLeft = [text attributeForName:@"Left"].stringValue.floatValue;
                CGFloat textTop = [text attributeForName:@"Top"].stringValue.floatValue;
                CGFloat textWidth = [text attributeForName:@"Width"].stringValue.floatValue;
                CGFloat textHeight = [text attributeForName:@"Height"].stringValue.floatValue;
                CGFloat fontSize = [text attributeForName:@"FontSize"].stringValue.floatValue;
            ...
            for (DDXMLElement *icon in iconsArray) {
                CGFloat iconLeft = [icon attributeForName:@"Left"].stringValue.floatValue;
                CGFloat iconTop = [icon attributeForName:@"Top"].stringValue.floatValue;
                CGFloat iconWidth = [icon attributeForName:@"Width"].stringValue.floatValue;
                CGFloat iconHeight = [icon attributeForName:@"Height"].stringValue.floatValue;
