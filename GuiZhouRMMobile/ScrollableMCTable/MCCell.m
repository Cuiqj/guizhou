...
- (id)initWithReuseIdentifier:(NSString*)reuseIdentifier cellHeight:(CGFloat)height andColumns:(NSArray*)columns {
	if (self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier]) {
		...
		for (int i = 0; i < [columns count]; i++) {
			...
			newCellLabel.tag = i;
            newCellLabel.userInteractionEnabled = YES;
            UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(handleLongPress:)];
            [longPress setMinimumPressDuration:0.2];
            [newCellLabel addGestureRecognizer:longPress];
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)gesture
{
    ...
    static float endX = 0;
    int column = gesture.view.tag;
    if (gesture.state == UIGestureRecognizerStateBegan) {
        startX = [gesture locationInView:self.superview.superview].x;
    } else if (gesture.state == UIGestureRecognizerStateChanged) {
        endX = [gesture locationInView:self.superview.superview].x;
        if ([self.delegate respondsToSelector:@selector(panOnCell:inColoumn:)]) {
            [self.delegate panOnCell:endX-startX inColoumn:column];
        }
    } else if (gesture.state == UIGestureRecognizerStateEnded ||
               gesture.state == UIGestureRecognizerStateCancelled) {
        if ([self.delegate respondsToSelector:@selector(clearLastPos)]) {
            [self.delegate clearLastPos];
        }
        startX = 0;
        endX = 0;
    }
