...
@interface MCTableScrollContainer (Private) <MCCellDelegate>
...
static float _lastTouchX = 0;

#pragma mark - MCCellDelegate
- (void)panOnCell:(float)x inColoumn:(int)column
{
    int originWidth = [self.columns[column] intValue];
    if (x > 0 || originWidth > 50) {
        [self changeWidth:originWidth+x-_lastTouchX inColumn:column];
    }
    _lastTouchX = x;
}
